...
signal order_progress_changed(progress: Array)  # M3-3.3: 订单进度 [{role,star,count,have}]
...
# M3-3.3: 猫咪委托（空=普通对局；订单模式下胜利条件替换为凑齐订单需求）
var active_order: Dictionary = {}
# M4 埋点：本局狂欢选择记录 / 付费撤销次数
var frenzy_modes_used: Array = []
var undo_paid_count: int = 0
...
func start_new_game(level: int = BoardGameData.BoardLevel.LV1, cat: String = "你的猫", twist_id: String = "", order: Dictionary = {}) -> void:
	"""初始化新一局：随机选链→初始掉落→生成器满。
	twist_id 非空时叠加当日变异（M3-3.2）；order 非空进入委托模式（M3-3.3），
	胜利条件替换为凑齐订单需求；测试/模拟不传保持基线规则。"""
	randomize()
	board_level = level
	cat_name = cat
	active_twist_id = twist_id
	active_twist = BoardTwists.get_twist(twist_id)
	active_order = order.duplicate(true)
	frenzy_modes_used = []
	undo_paid_count = 0
...
func merge_items(pos_a: Vector2i, pos_b: Vector2i) -> bool:
	...
	# 检查通关：普通局=主链⭐5出现；委托局=凑齐订单需求（M3-3.3）
...
func get_order_progress() -> Array:
	"""订单各需求项的当前进度：[{role, star, count, have}]"""
	var progress: Array = []
	for req in active_order.get("requirements", []):
		var chain: int = current_main_chain if String(req["role"]) == "main" else current_sub_chain
		var star := int(req["star"])
		var have := 0
		for pos in grid:
			var item: BoardItem = grid[pos]
			if item.chain == chain and item.star == star:
				have += 1
		progress.append({"role": req["role"], "star": star, "count": int(req["count"]), "have": have})
	return progress


func is_order_fulfilled() -> bool:
	"""棋盘上同时凑齐订单全部需求即完成"""
	if active_order.is_empty():
		return false
	for entry in get_order_progress():
		if int(entry["have"]) < int(entry["count"]):
			return false
	return true
...
func click_generator() -> bool:
	...
	_check_mischief()
	# M3-3.3: 限时委托——超出生成次数上限仍未交付即失败
	var click_limit := int(active_order.get("click_limit", 0))
	if click_limit > 0 and generator_click_count >= click_limit and game_state == BoardGameData.GameState.PLAYING and not is_order_fulfilled():
		game_state = BoardGameData.GameState.LOST
		_settle_unused_guards()
...
func serialize_state() -> Dictionary:
	...
	return {
		...
		"frenzy_modes_used": frenzy_modes_used.duplicate(),
		"undo_paid_count": undo_paid_count,
	}
...
func deserialize_state(data: Dictionary) -> void:
	...
	for t in data.get("mischief_triggers", []):
		mischief_triggers.append(int(t))
	# M3-3.3 / M4: 恢复委托与埋点计数
	var order_data: Variant = data.get("active_order", {})
	active_order = order_data.duplicate(true) if order_data is Dictionary else {}
	frenzy_modes_used = []
	for m in data.get("frenzy_modes_used", []):
		frenzy_modes_used.append(int(m))
	undo_paid_count = int(data.get("undo_paid_count", 0))
	undo_stack.clear()
...
func trigger_frenzy(mode: int = BoardGameData.FrenzyMode.GUARD) -> bool:
	...
	match mode:
		...
		_:
			return false
	frenzy_modes_used.append(mode)  # M4 埋点
