...
var highest_star_achieved: int = 1  # 本局已合成的最高星级（初始1=⭐1）
...
func merge_items(pos_a: Vector2i, pos_b: Vector2i) -> bool:
	...
	if triple_list.size() >= 2:  # 加上自身合进来的，共构成三连合
...
func serialize_state() -> Dictionary:
	...
	return {
		...
		"_last_merge_time": _last_merge_time,
...
func deserialize_state(data: Dictionary) -> void:
	...
	_last_merge_time = float(data.get("_last_merge_time", 0.0))
